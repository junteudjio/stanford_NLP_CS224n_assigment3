...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            U_z = tf.get_variable(name='U_z', shape=[self.input_size, self._state_size], dtype=tf.float32,
                                  initializer=tf.contrib.layers.xavier_initializer())
            U_r = tf.get_variable(name='U_r', shape=[self.input_size, self._state_size], dtype=tf.float32,
                                  initializer=tf.contrib.layers.xavier_initializer())
            U_o = tf.get_variable(name='U_o', shape=[self.input_size, self._state_size], dtype=tf.float32,
                                  initializer=tf.contrib.layers.xavier_initializer())

            W_z = tf.get_variable(name='W_z', shape=[self._state_size, self._state_size], dtype=tf.float32,
                                  initializer=tf.contrib.layers.xavier_initializer())
            W_r = tf.get_variable(name='W_r', shape=[self._state_size, self._state_size], dtype=tf.float32,
                                  initializer=tf.contrib.layers.xavier_initializer())
            W_o = tf.get_variable(name='W_o', shape=[self._state_size, self._state_size], dtype=tf.float32,
                                  initializer=tf.contrib.layers.xavier_initializer())

            b_z = tf.get_variable(name='b_z', shape=[self._state_size, ], dtype=tf.float32,
                                initializer=tf.contrib.layers.xavier_initializer())
            b_r = tf.get_variable(name='b_r', shape=[self._state_size, ], dtype=tf.float32,
                                initializer=tf.contrib.layers.xavier_initializer())
            b_o = tf.get_variable(name='b_o', shape=[self._state_size, ], dtype=tf.float32,
                                initializer=tf.contrib.layers.xavier_initializer())

            z_t = tf.nn.sigmoid(tf.matmul(x_t, U_z) + tf.matmul(h_t_1, W_z) + b_z, name='z_t')
            r_t = tf.nn.sigmoid(tf.matmul(x_t, U_r) + tf.matmul(h_t_1, W_r) + b_r, name='r_t')
            o_t = tf.nn.tanh(tf.matmul(x_t, U_o) + tf.matmul(r_t * h_t_1, W_o) + b_o, name='o_t')


            h_t = tf.add(z_t * h_t_1, (1. - z_t) * o_t, name='h_t')
            new_state = h_t
            ### END YOUR CODE ###
