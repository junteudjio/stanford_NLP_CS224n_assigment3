...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    def get_word_features(word_idx, sentence):
        if word_idx < 0:
            return start
        elif word_idx >= len(sentence):
            return  end
        else:
    ...
    ### YOUR CODE HERE (5-20 lines)
        for idx, word in enumerate(sentence):
            window_label = labels[idx]
            window_feature = []
            for word_idx in xrange(-window_size + idx, idx + window_size + 1):
                window_feature.extend(get_word_features(word_idx, sentence))

            window_data = (window_feature, window_label)
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.nn.embedding_lookup(params=embedded, ids=self.input_placeholder, name='pre_concat_embeddings')
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable(name='W', dtype=tf.float32,
                            shape=[self.config.embed_size * self.config.n_window_features, self.config.hidden_size],
                            initializer=tf.contrib.layers.xavier_initializer())
        b1 = tf.Variable(tf.zeros(shape=[1, self.config.hidden_size]), dtype=tf.float32, name='b1')
        h = tf.nn.relu(tf.matmul(x, W) + b1, name='h')


        h_drop = tf.nn.dropout(h, keep_prob=dropout_rate, name='h_drop')
        U = tf.get_variable(name='U', dtype=tf.float32,
                            shape = [self.config.hidden_size, self.config.n_classes],
                            initializer=tf.contrib.layers.xavier_initializer())
        b2 = tf.Variable(tf.zeros(shape=[1, self.config.n_classes]), dtype=tf.float32, name='b2')

        pred = tf.add(tf.matmul(h_drop, U), b2, name='logits')
        ### END YOUR CODE
        return pred
    ...
    def add_loss_op(self, pred):
        ...
        xentropy = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder,
                                                                  logits=pred, name='xentropy')
    ...
    def add_training_op(self, loss):
        ...
        optimizer = tf.train.AdamOptimizer(learning_rate=self.config.lr, name='AdamOptimizer')
        train_op = optimizer.minimize(loss, name='loss')
        ### END YOUR CODE
        return train_op
