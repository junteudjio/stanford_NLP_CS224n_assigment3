...
def pad_sequences(data, max_length):
    ...
    sentence_labels_mask = namedtuple('sentence_labels_mask', ['sentence', 'labels', 'mask'])

    for sentence, labels in data:
        ### YOUR CODE HERE (~4-6 lines)
        offset = max_length - len(sentence)
        mask = [True] * len(sentence)
        new_labels = labels[:max_length]
        new_sentence = sentence[:max_length]
        mask = mask[:max_length]
        if offset > 0:
            mask.extend([False] * offset)
            new_sentence.extend([zero_vector for _ in xrange(offset)])
            new_labels.extend([zero_label for _ in xrange(offset)])
        ret.append(sentence_labels_mask(sentence=new_sentence, labels=new_labels, mask=mask))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(embeddings,
                                shape=[-1, self.max_length, self.config.n_features * self.config.embed_size],
                                name='embeddings')
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE (~4-6 lines)
        U = tf.get_variable(name='U', dtype=tf.float32, shape=[self.config.hidden_size, self.config.n_classes],
                            initializer=tf.contrib.layers.xavier_initializer())
        b2 = tf.Variable(tf.zeros(shape=[self.config.n_classes,], dtype=tf.float32), dtype=tf.float32, name='b2')
        ### END YOUR CODE

        # h_0:
        h_t = tf.Variable(tf.zeros(shape=[1, self.config.hidden_size], dtype=tf.float32), dtype=tf.float32)
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o_t, h_t = cell(x_seqs[time_step], h_t)
                o_drop_t = tf.nn.dropout(o_t, keep_prob=dropout_rate)
                logits_t = tf.matmul(o_drop_t, U) + b2
                preds.append(logits_t)
                ### END YOUR CODE

        # Make sure to reshape @preds here.
        ### YOUR CODE HERE (~2-4 lines)
        preds = tf.transpose(tf.stack(preds), perm=[1, 0, 2])
        ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        xentropy = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=preds, labels=self.labels_placeholder, name='xentropy')
        loss = tf.reduce_mean(tf.boolean_mask(xentropy, self.mask_placeholder), name='loss')
        ### END YOUR CODE
        return loss
    ...
    def add_training_op(self, loss):
        ...
        adam_optimizer = tf.train.AdamOptimizer(self.config.lr)
